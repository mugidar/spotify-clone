...
const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <Dialog.Root open={isOpen} defaultOpen={isOpen} onOpenChange={() => onChange(isOpen)}>
      <Dialog.Trigger asChild>
        <button className="Button violet">{title}</button>
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay  onClick={() => onClose()}  className="DialogOverlay bg-neutral-500/20 backdrop-blur-sm fixed inset-0" />
        <Dialog.Content className="DialogContent fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90w] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
          ...
          <Dialog.Close asChild>
            <button onClick={() => onClose()} className="IconButton text-neutral-400 hover:text-white absolute top-[10px] right-[10px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:outline-none" aria-label="Close" >
              <IoMdClose />
            </button>
          </Dialog.Close>
        </Dialog.Content>
